 ...
 *
 * 背景：X server 是终端侧 app_process（CmdEntryPoint，经 anwind-x11 拉起），
 ...
 *   语义下系统不冻结前台服务所在进程）；
 * - 每 5 秒核查 X server binder 存活（anwind-x11-stop / 进程被杀都会
 *   置 binder dead）→ 会话结束自动停止自身、撤掉常驻通知；
 * - 通知动作："X11 设置"（打开 X11 窗口 + 弹设置面板）、点击正文回到
 *   AnWind 桌面。长按通知 = 系统级通知设置入口。
 */
class X11KeepAliveService : Service() {
    ...
    companion object {
        ...
        const val ACTION_OPEN_SETTINGS = "com.anwind.x11.KEEPALIVE_OPEN_SETTINGS"
        const val ACTION_OPEN_WINDOW = "com.anwind.x11.KEEPALIVE_OPEN_WINDOW"
    ...
    /** 会话存活巡检：X server binder 死亡（anwind-x11-stop/被杀）→ 自停。 */
    ...
    /** 回到 AnWind 桌面（launcher intent；singleTask 不会重复建栈）。 */
    ...
    private fun startForegroundWithNotification() {
        ...
        val notification: Notification = builder
            ...
            .setContentText("X11 桌面会话常驻保护 · 点按回到 AnWind")
