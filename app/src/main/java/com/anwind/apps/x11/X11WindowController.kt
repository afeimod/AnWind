 ...
 *
 * 终端侧 `anwind-x11` 启动的 X server 进程（CmdEntryPoint，app_process）
 * 每秒广播一次 ACTION_START，附带自己 binder（ICmdEntryInterface）。本对象：
 *
 * 1. 接收广播（AnWindApp 注册的动态接收器回调），取出 binder 暂存；
 * 2. 在 AnWind 桌面内打开/聚焦 "X11 桌面" 浮动窗口（不再拉起独立全屏
...
object X11WindowController {
    ...
    /**
     * AnWindApp 动态接收器入口：暂存 binder 并打开桌面窗口。
    ...
    private fun openOrFocusWindow(context: Context) {
        ...
        val appDef = com.anwind.core.window.AppRegistry.get(APP_ID)
        wm.open(
            appId = APP_ID,
        ...
        Log.i(TAG, "已在 AnWind 桌面打开 X11 窗口")
    }
    ...
    fun connectLorieView(view: LorieView) {
        ...
        fun attempt() {
            ...
            // 会话已死（anwind-x11-stop 或进程被杀）：不再空转，回 Idle。
