...
object X11ResolutionLink {
    ...
    private const val RES_FILE = "$RES_DIR/.anwind-x11-res"
    ...
    /** 上一次 Toast 提示的握手值（去重，避免 anwind-x11/glibc-runner 交替写文件时刷屏）。 */
    ...
    fun start(context: Context) {
        ...
        observer = object : FileObserver(RES_DIR, CLOSE_WRITE or MOVED_TO or CREATE) {
            override fun onEvent(event: Int, path: String?) {
                if (path != null && path != ".anwind-x11-res") return
     ...
     * 桌面内容更多）。与 native/exact 一样经 regenerate 即时生效；
