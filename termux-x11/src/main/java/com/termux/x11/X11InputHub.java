...
/**
 * AnWind（v2.22.3 fix10）：X11 输入中枢（app 进程级单例）。
 *
 * 解决三个问题：
 ...
 *    SharedPreferences（anwind_gamepad_profile_id），下次开启直接恢复。
 *
...
public class X11InputHub implements WinHandler.Host {
    ...
    private static final String KEY_PROFILE_ID = "anwind_gamepad_profile_id";
    ...
    /**
     * AnWind（v2.22.3 fix11b）：当前活跃的 X11 渲染视图（LorieView）。
     ...
     *
     * AnWind hotfix：start() 内含 InetAddress.getLocalHost()（DNS 解析，
    ...
    // ============================================================
    // AnWind（v2.22.3 fix11b）：桌面虚拟手柄 → X11 输入桥
    // ============================================================
