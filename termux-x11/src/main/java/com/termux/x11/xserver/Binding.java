...
/**
 * Binding —— 外置手柄/输入绑定枚举（termux-x11 新 xserver 包，结构对齐 Winlator Cmod 上游）。
 *
 * 与上游的差异：本集成把 winlator 自带 XServer 显示栈剔除了
 * （显示端由 AnWind 的 lorie X server 承担），因此去掉对
 * com.winlator.cmod.xserver.XKeycode 的依赖，仅保留键名/序列化语义
 * （toJSONObject 使用 binding.name()，与上游配置文件兼容）。
 */
public enum Binding {
