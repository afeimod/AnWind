...
// AnWind：类型改由 :winlator 模块自身提供，不再依赖 XServerDisplayActivity/UI 层
...
public class WinHandler {
    ...
    /** AnWind：宿主上下文（用于偏好读取；null = 使用默认值）。 */
    private final android.content.Context activity;
    ...
    /** AnWind：无宿主构造（偏好全走默认值；可稍后 attachContext）。 */
    public WinHandler() {
        this.activity = null;
    }

    public WinHandler(android.content.Context context) {
        this.activity = context != null ? context.getApplicationContext() : null;
        if (this.activity != null)
            preferences = PreferenceManager.getDefaultSharedPreferences(this.activity);
    }

    /** 延迟绑定宿主上下文（X11InputHub 无参构造后调用）。 */
    ...
    private void handleRequest(byte requestCode, final int port) {
        switch (requestCode) {
            ...
            case RequestCodes.GET_GAMEPAD: {
                ...
                // AnWind：虚拟手柄层已随 UI 重构移除，仅物理手柄（外部手柄直连）
            ...
            case RequestCodes.CURSOR_POS_FEEDBACK: {
                ...
                // AnWind：lorie X server 自带光标管理，无需回写指针位置（仅消费报文）
                break;
            }
            default: {
    ...
    public void sendGamepadState() {
        ...
        for (final int port : gamepadClients) {
            addAction(() -> {
                ...
                if (enabled) {
                    sendData.putInt(currentController.getDeviceId());
                    GamepadState state = currentController.state;
...
//    }
    ...
    public void initializeController() {
        currentController = ExternalController.getController(0);
        if (currentController != null) {
            if (activity != null) currentController.setContext(activity); // AnWind: Context 版
